    ...
    <form class="card border-0 shadow-sm mb-3" method="get" action="${pageContext.request.contextPath}/admin/revenue">
        <div class="card-body row g-3 align-items-end">
            ...
            <div class="col-sm-3 d-flex gap-2">
                ...
                <a class="btn btn-outline-light" href="${pageContext.request.contextPath}/admin/revenue?period=${period}&from=${from}&to=${to}&export=csv">
                    Export CSV
                </a>
            </div>
        </div>
...
<script>
    // labels & values từ JSTL
    const labels = [
    <c:forEach var="l" items="${labels}" varStatus="st">
        ${st.first?'':','}'${fn:escapeXml(l)}'
    </c:forEach>
    ];
    const values = [
    <c:forEach var="v" items="${values}" varStatus="st">
        ${st.first?'':','}${v}
    </c:forEach>
    ];
